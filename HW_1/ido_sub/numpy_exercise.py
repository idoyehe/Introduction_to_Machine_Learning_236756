...
def one_hot_5_of_10():
    ...
    """
    return np.concatenate((np.zeros(4, dtype=int), np.array([1]), np.zeros(5, dtype=int)))
...
def get_size_properties(x):
    ...
    """
    return np.shape(x) + (np.size(x), np.ndim(x))
...
def column_sum(x):
    ...
    """
    return x.reshape(x.shape).sum(axis=0)
...
x_list = X.tolist()
y_list = Y.tolist()"""
    print(timeit.timeit(stmt="mat_mul_pure_python(x_list, y_list)", setup=setup1, number=10))
